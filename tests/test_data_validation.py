...
def test_sample_data_no_missing_values(sample_data):
    """Test that sample data has no missing values"""
    assert not sample_data.isnull().any().any()


def test_sample_classification_data(sample_classification_data):
...
def test_sample_regression_data(sample_regression_data):
...
def test_csv_data_loading(data_dir):
...
def test_parquet_data_loading(data_dir):
